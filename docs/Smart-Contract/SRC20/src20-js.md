...
:::tip
I recommend that you download the [Long Term Support](https://nodejs.org/en/download/) 
:::
...
For modern JavaScript development, you really owe it to yourself to try [VSCode](https://code.visualstudio.com/). `sbitjs` comes with static type definitions for its API, and with VSCode you get some of the most useful IDE features (e.g. type-accurate autocomplete) without the UX bloat:

## Setup The SRC20 CLI Project
...
* `call` (or "query"): executes contract code on your own local `sbitd` node as a "simulation", returning results, but not changing the blockchain. This is free.
...
Next, we are going to mint some new tokens using sbitjs. And because minting token changes the blockchain, we'll use `send`.
...
As we've learned in [The Owner UTXO Address](src20-token#the-owner-address), SBIT doesn't really have the idea of an "account". The `msg.sender` is the address of whatever UTXO that was used to pay for the transaction.
...
In the above code, the third argument of `send` allows you to specify the `msg.sender`.
...
Let's use sbitjs to subscribe to the stream of contract events, so we can react in a timely manner when a transfer occurs. The code:
...
> If you are running your own sbitd node instead of the provided docker image, you'll need to enable `-logevents` for events logging to work.

# Conclusion

In this chapter we've developed a simple NodeJS CLI tool to interact with an SRC20 contract.

+ sbitjs is a Promise-based API. Use async/await to write clean asynchronous code.
+ `call` is like "query", `send` is like "commit".
+ Use `senderAddress` to in `call` or `send` to specify the `msg.owner`.

Now that you know how to use sbitjs, you are ready to build a DApp, and be on your way to fame and riches!
