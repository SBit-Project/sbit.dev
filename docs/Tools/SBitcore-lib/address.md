---
title: SBIT Address
description: Represents a SBIT address. Addresses are the most popular way to make SBIT transactions.
...
To be able to receive SBITs an address is needed, but in order to spend them a private key is necessary. Please take a look at the [`PrivateKey`](privatekey) docs for more information about exporting and saving a key.  
...
You can also instantiate an Address from a String, [PublicKey](publickey), or [HDPublicKey](hierarchical), in case you are not the owner of the private key.
...
A pay-to-script-hash multisignature Address can be instantiated from an array of [PublicKeys](publickey).
