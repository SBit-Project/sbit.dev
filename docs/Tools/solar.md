...
```
go get -u github.com/SBit-Project/solar/cli/solar
```
...
Start sbitd in regtest mode:

```shell
sbitd -regtest -rpcuser=user -rpcpassword=pass
```

Use env variable to specify the local sbitd RPC node:
...
You can run sbitd with docker, which comes bundled with solar (and `solc`):
