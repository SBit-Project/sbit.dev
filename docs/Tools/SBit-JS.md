...
> To run sbitd for the test network (testnet):
...
:::note
The default JSON-RPC credential is "sbit:test", running on port 22002
:::
...
Executes contract method on your own local sbitd node as a "simulation" using `callcontract`. It is free, and does not actually modify the blockchain.
...
All RPC methods supported by sbitd.
