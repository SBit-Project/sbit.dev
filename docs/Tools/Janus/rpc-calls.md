---
title: RPC Calls
description: A full list of all the ETH-RPC calls that are supported by Janus, as well as
...
* http://localhost: - Docker endpoint
* http://localhost:22401 - Local endpoint for `make run-janus`
...
When requests are made that act on the state of sbit, the last default block parameter determines the height of the block.
...
Returns `true` if the wallet is **staking sbit** or not
...
Returns the SBIT amount per gas unit.
...
* `Quantity` - hex representing the SBIT amount per gas unit
...
Returns a list of the hex addresses corresponding to the base58 SBIT addresses owned by the wallet
...
**NOTE: The SBIT RPC calls used in Janus for translation do not make use of the block parameter, so including it in this call won't affect the return value as it will always return the most current indexed balance.**
...
**NOTE:**  This call is set to a default value because SBIT does not use a nonce to determine the transaction count.
...
1. `Object` - The transaction object
   ...
   * `gasPrice:` `QUANTITY` \(optional, default: TBD\) Integer of the gasPrice used for each paid gas in SBIT
...
**NOTE:** This call does not have the nonce option, given that nonces are not used in SBIT.
...
* `value`: `QUANTITY` - value transferred in SBIT
...
2. `QUANTITY` float value for **minimum amount** in SBIT, if the minimum amount is greater than the sum of all available UTXOs the call will return with an error message **"required minimum amount is greater than total amount of UTXOs"**
